...
const { monthEnd, priceTicks, timeStep, seekIndex, zonedTime, zonedCandidates } = require('../chart-utils');
...
test('month boundaries clamp short months and handle leap years and year rollover', () => {
  for (const [start, end, days] of [
    ['2026-08-09', '2026-09-09', 31], ['2026-01-31', '2026-02-28', 28],
    ['2028-01-31', '2028-02-29', 29], ['2026-12-15', '2027-01-15', 31],
  ]) {
    const range = dateRange(start);
    assert.equal(new Date(range.end).toISOString().slice(0, 10), end);
    assert.equal(range.days, days);
    assert.equal(monthEnd(range.start), range.end);
  }
  assert.equal(dateRange('2026-08-09', '1').days, 1);
  for (const date of ['2026-02-30', 'bad', undefined]) assert.throws(() => dateRange(date));
  assert.throws(() => dateRange('2026-08-09', '7'));
});
...
test('month loader paginates every minute and preserves UTC bounds', async t => {
  let requests = 0;
...
test('empty days are allowed but exchange failures are surfaced', async t => {
  ...
  await assert.rejects(loadCandles('2026-03-02', '1'), /429/);
});
