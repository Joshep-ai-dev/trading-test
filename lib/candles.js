...
function dateRange(date, days) {
  ...
  if (days !== undefined && String(days) !== '1') throw new Error('Only days=1 is supported; omit days for one month');
  ...
  const end = days === undefined ? next.getTime() : start + DAY;
...
async function loadCandles(date, days) {
  ...
  const chunks = [];
  // Small batches keep full-month loads within exchange request limits.
  for (let start = range.start; start < range.end; start += 3 * DAY) {
    const dates = [start, start + DAY, start + 2 * DAY].filter(day => day < range.end);
    chunks.push(...await Promise.all(dates.map(loadDay)));
  }
